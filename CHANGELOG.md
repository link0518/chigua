...
- 新增匿名「队伍招募」：发布者选择自己的 DPS 心法并填写正文，其他身份选择自己的 DPS 心法后可发起仅双方可见的密聊；支持联系方式双向同意解锁、招募通知、举报与独立限流，招募发布 24 小时后会与关联密聊一同自动结束。藏剑在目录中合并为一个选项，底层保留问水诀与山居剑意源记录。
- 新增招募后台治理：按举报查看有限密聊证据，支持招募下架/恢复、会话锁定/解锁、消息与联系方式软删除/恢复、举报处理、封禁和隐私审计；同步新增招募数据模型、API、权限与服务端回归测试。
...
- 招募申请只建立密聊，不再生成“收到新的申请”通知；申请者实际发送消息后才提醒对方，历史申请通知也不再展示或计入未读。
- 收敛招募轮询负载：密聊消息响应可同时携带联系方式，招募通知并入通用通知轮询；匿名高频读取跳过无业务依赖的 Session 续期，稳定身份映射最多每 5 分钟刷新一次，并为开放招募过期检查增加专用部分索引。
...
- 全局状态拆分为 AppShell、Feed、Content、UserPreferences、Admin、ToastUI 和稳定 AppActions 域；热门卡片、路由渲染器和 Toast 项增加定向 memo，列表更新仅替换目标帖子对象。
...
- 修复对方同意交换联系方式后，已打开的密聊窗口只更新按钮状态、没有即时提示的问题；下一次密聊轮询会提示联系方式已解锁并自动展开联系方式面板，首次进入历史已解锁会话时不会重复提示。
- 收紧招募后台资源治理边界：招募、会话、消息和联系方式动作必须匹配举报目标类型，只有举报明确绑定的证据消息保留治理例外，不能再借 `post_id`、`thread_id` 等上下文字段扩大处置范围。
- 升级 `dompurify` 至 3.4.12、`body-parser` 至 1.20.6、`brace-expansion` 至 1.1.16，清除完整依赖树中的已知 npm 安全审计告警。
- 修复首页分享详情在 `homeIndex` 超出已加载范围时可能连续补页、旧请求覆盖新列表和临时详情污染分页偏移的问题；补页失败后会按帖子 ID 降级加载，临时帖子命中自然分页后恢复正确顺序，并避免解析期间展示、计浏览或互动错误帖子。
- 修复首页刷新与点赞、收藏、评论、加精申请或删除操作并发时旧响应回滚新状态，以及失效补页导致“下一个瓜”持续加载的问题；首页写操作会统一淘汰较早请求。
- 修复同一帖子跨首页、热门和精华切换时可能重复提交互动请求，以及收藏页取消收藏后下一页跳项的问题；热门缓存按字段协调互动写入并保留会话删除墓碑，隐藏标签页不再按内容 TTL 持续后台刷新。
- 修复首页、评论、精华和收藏接口对非法分页参数处理不一致、收藏总数包含不可见帖子的问题；公开热门搜索不再支持通过 IP 或浏览器指纹检索匿名帖子。
- 为投稿页补齐 `/submit` 路由，修复从其它页面进入投稿后被立即解析回首页，以及移动端 Markdown 表情面板按返回键时连投稿页一起退出的问题；图片查看器补齐真实模态框内的可聚焦关闭按钮、焦点约束与跨断点恢复，清理无图幽灵 history，并统一处理卸载后的页面滚动锁。
- 修复全局状态或顶栏局部状态变化会穿透重渲染热门页的问题，以及删除非热门帖子时错误递减热门总数的问题。
- 修复同一热门帖子快速连续点赞、点踩或收藏可能产生并发竞态，以及 Toast 新增时旧提示被重复计时、卸载后定时器未清理的问题。
- 修复快速切换搜索条件或浏览器前进后退时，较早请求可能覆盖较新搜索结果并提前结束加载状态的问题。
- 修复 Wiki CSS 拆分时误带走首页减弱动态效果、移动端 Markdown 图片与全局滚动条规则的问题，确保未访问 Wiki 时主站行为不变。
...
- 修复移动评论编辑器逐键触发整棵评论树重渲染和焦点抖动的问题；草稿改为本地编辑、防抖缓存，并补齐 Back、刷新、跨断点、切帖及慢请求返回时的恢复与隔离保护。
...
- 修复后台移动端登录首屏在会话检查与字体切换时的布局偏移：复用稳定登录卡布局并改用系统中文字体，消除 CLS 与 WebFont 延迟。
