...
test('谣言举报必须填写依据，并在每次新增待审记录时推送 webhook', () => {
  const db = createDb();
  const hookState = { hiddenMessages: [], rumorMessages: [] };
...
test('同一目标重复增加谣言举报时会再次推送待审 webhook，并带上最新待处理数量', () => {
  const db = createDb();
  const hookState = { hiddenMessages: [], rumorMessages: [] };
  ...
  assert.equal(hookState.rumorMessages.length, 2);
  assert.equal(hookState.rumorMessages[1].pendingReportCount, 2);
  assert.equal(hookState.rumorMessages[1].evidence, '第二份证据');
