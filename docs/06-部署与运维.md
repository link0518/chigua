...
`NODE_OPTIONS=--max-old-space-size=2048` 运行构建；手动构建可使用：

```bash
NODE_OPTIONS=--max-old-space-size=2048 npm run build
```

若仍然 OOM，可先给服务器添加 2G swap，或临时调大脚本参数：
...
- 创建 `.env.local`（若不存在）
- `npm install --include=optional` + 带 `NODE_OPTIONS=--max-old-space-size=2048` 的 `npm run build`
