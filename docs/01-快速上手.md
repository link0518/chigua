...
- `VITE_IMGBED_BASE_URL` / `VITE_IMGBED_TOKEN`：前端图片上传（图床）配置；不配不影响发帖/评论，但图片上传会提示未配置（投稿页与移动端评论弹窗的图片/表情按钮为仅图标）
  - 为了减少刷图行为，前端对“图片上传”做了轻量限频：同一浏览器会话内约 30 秒最多 3 次；超出会提示稍等再试
