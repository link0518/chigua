...
- 首页支持“单帖 / 列表”双模式切换，默认进入单帖模式，并通过 `localStorage` 记住用户上次选择。
- `/post/:id` 与 `/post/:id?comment=...` 会强制进入单帖聚焦视图，用于分享帖子和直达评论。
- 列表模式用于高密度扫帖：首屏默认加载 `20` 条，底部“继续加载更多帖子”每次再追加 `20` 条，避免一次性加载过多导致卡顿。
- 列表卡片点击正文会在新标签页打开对应单帖，并在链接中携带列表位置；详情页内“上一个瓜 / 下一个瓜”会沿着原列表顺序继续浏览。
- 带 `comment` 参数的单帖链接只会自动展开一次评论弹窗；用户手动关闭后，后续点赞、收藏、刷新等状态更新不会重复拉起弹窗。
...
- `types.ts`：共享类型定义
- `server/`：后端（路由、鉴权、SQLite、管理逻辑，含 `identity-service.js` 身份归一）
...
- 前端对发帖、评论、点赞、举报等风控敏感接口，仍会按需附带 `X-Client-Fingerprint`。
- 服务端会把 Cookie 身份与 legacy 指纹关联到 `identity_aliases`，并在封禁判断、通知读取、收藏/点赞状态、连续登录彩蛋身份识别等场景按“身份集合”聚合，而不是只看单次指纹。
